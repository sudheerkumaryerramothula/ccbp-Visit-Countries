...
import VisitCountryListCard from '../VisitCountryListCard'
...
class Lists extends Component {
  ...
  visitButtonClicked = id => {
    ...
    const visitCountry = visitedCountries.map(eachCountry => {
    ...
    this.setState({visitedCountries: visitCountry})
  }

  onClickRemoveButton = id => {
    const {visitedCountries} = this.state

    const removeCountry = visitedCountries.map(eachCountry => {
      if (eachCountry.id === id) {
        const updatedList = {
          ...eachCountry,
          isVisited: !eachCountry.isVisited,
    ...
    this.setState({visitedCountries: removeCountry})
  }

  render() {
    const {visitedCountries} = this.state
    const countriesList = visitedCountries.filter(
        ...
        <div className="container">
          ...
          <ul className="visit-countries-list">
            {visitedCountries.map(eachCountry => (
              <VisitCountryListCard
                key={eachCountry.id}
                countryDetails={eachCountry}
                visitButton={this.visitButtonClicked}
              />
            ))}
          </ul>
          ...
          {countriesList.length < 1 ? (
            <div className="no-list-container">
              <p className="no-views-content">No Countries Visited Yet</p>
            </div>
          ) : (
            <ul className="visited_countries-list">
              {countriesList.map(eachCountry => (
                <VisitedCountryCard
                  key={eachCountry.id}
                  visitedCountryDetails={eachCountry}
                  clickedRemoveButton={this.onClickRemoveButton}
                />
              ))}
            </ul>
          )}
